...
1. Do not promote or full-gate-check `2026050603`, `2026050605`, or the `2026050608` smoke diagnostic; none improved the accepted AI. This round improved diagnosis, not playing strength. The latest evidence says behavior-visible changes remain unsafe, and wrapper-changed samples are not simply bad samples.
2. Next implementation target: add a runtime wrapper weighting option instead of another binary filter. A conservative first version should keep all trainable samples but reduce tacticalChanged sample weight, optionally leave staminaConserved/criticalRegulated samples at full weight, and record the weight mode in candidate metadata/metrics/history.
3. After that, run a trace-gated search that compares default runtime training, `--runtime-survivors-only`, and the new wrapper-weighted mode on the same open/mixed hypothesis. Promote only if both standard and holdout gates improve without stamina/critical wrapper-rate regression and with nonzero holdout final-action change.
4. Use `runtimeDecisionOutcomes` before changing reward or baseline logic. If a wrapper category has positive advantage but causes gate regression, treat it as a credit-assignment problem; if it has consistently negative advantage, down-weight it more aggressively.
