...
每次完成工作后，去掉project.md中过时无用的内容，如果本轮工作ai没有提高，反思原因。并制定下一步计划。
...
2. Use `--trace-gate` in the next short search so behavior visibility is captured during ranking instead of as a manual after-step. The bridge now counts final-action changes and wrapper-rate deltas on standard/holdout seeds; the next improvement target is to use those metrics to choose hypotheses, not to keep running blind PPO sweeps.
