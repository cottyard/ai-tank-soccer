...
1. Do not promote or full-gate-check `2026050603`, `2026050605`, `2026050608`, or `2026050610`; none improved the accepted AI. This round added a safer objective variant, but the quick candidates were still runtime behavior-invisible.
2. The next search should not run a large parallel trace grid. Use one sequential script or a small search grid with explicit modes `none`, `runtime-survivors-only`, and `tactical-downweight`, fewer seeds first, and kill/resume handling so timeout does not leave background `tsx` processes.
3. Because `2026050610` did not change final runtime actions at all, increase behavior pressure before another full gate: try longer training (`matches>=240`, `frames=180`) or a slightly higher temperature/clip only inside the three-mode comparison, and require nonzero holdout final-action change before considering promotion.
4. Use `runtimeDecisionOutcomes` before changing reward or baseline logic. If tacticalChanged samples remain negative on the chosen recipe, compare `tactical-downweight` against a stronger tactical downweight; if they become positive, treat wrapper rewrite as credit assignment rather than noise.
