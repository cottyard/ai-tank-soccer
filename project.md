...
1. Do not promote `2026050603`, `2026050605`, `2026050608`, `2026050610`, `2026050611`, `2026050612`, `2026050613`, `2026050614`, `2026050615`, or `2026050701`; none improved the accepted AI under full gates or the required `97/109` reduced trace gate.
...
3. Keep `--runtime-wrapper-modes none,runtime-survivors-only,tactical-downweight`, `--runtime-tactical-rewrite-weights`, and `--action-retention-weights` available for diagnostics, but stop spending mainline budget on open-start wrapper handling or action retention alone; the fixed implementations either stayed behavior-invisible or failed full standard gates.
...
5. The next training/search hypothesis should preserve behavior visibility while preventing early low-pressure full-forward regressions. Test candidates against a reduced trace gate that reports first-divergence actions on `97/109`; prefer candidates whose first divergences are not current `8` becoming candidate `5` in low-pressure states. Since global action retention changed weights without changing reduced-gate behavior and then failed full standard seed `31`, prioritize an explicit early-open-start safety slice, seed-31 regression analysis, or first-divergence-aware sample filtering before expanding architecture.
