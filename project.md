...
5. The next training/search hypothesis should preserve behavior visibility while preventing early low-pressure full-forward regressions. Test candidates against a reduced trace gate that reports first-divergence actions on `97/109`; prefer candidates whose first divergences are not current `8` becoming candidate `5` in low-pressure states. Consider adding a conservative KL/action-retention penalty or an early-open-start safety slice before expanding architecture.
