...
5. The next training/search hypothesis should preserve behavior visibility while preventing early low-pressure full-forward regressions. Test candidates against a reduced trace gate that reports first-divergence actions on `97/109`; prefer candidates whose first divergences are not current `8` becoming candidate non-forward actions in low-pressure states. Since global action retention changed weights without changing reduced-gate behavior and then failed full standard seed `31`, prioritize an explicit early-open-start safety slice or first-divergence-aware sample filtering before expanding architecture.
