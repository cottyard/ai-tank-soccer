...
1. Do not promote `2026050603`, `2026050605`, `2026050608`, `2026050610`, `2026050611`, or `2026050612`; none improved the accepted AI under full gates.
...
3. Keep `--runtime-wrapper-modes none,runtime-survivors-only,tactical-downweight` as the preferred small-grid mechanism for wrapper objectives, but stop increasing temperature/clip on open starts unless the reduced gate includes `97/109` and shows no holdout-risk signal.
4. Use `runtimeDecisionOutcomes` before changing reward or baseline logic. TacticalChanged samples were negative in `2026050611/0612`; if future metrics keep that pattern, test a stronger tactical downweight or exclude tactical-rewritten samples only in starts where survived samples carry positive advantage.
5. Keep learned baselines and weighted league sampling available, but treat them as variants to search only after the behavior-visibility and holdout-risk problem is addressed. Keep runtime-action Rust parity covered by tests whenever `neuralStrategy`, stamina regulation, tactical rollout, or physics changes.
