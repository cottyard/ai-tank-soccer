...
4. The next useful implementation work should add a seed-level trace analyzer that reports where policy argmax changes are lost by tactical rollout and stamina guards, not just aggregate histograms. Use it on holdout `97/109` before changing reward or baseline logic again.
