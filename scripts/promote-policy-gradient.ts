...
function parseTrainingOptions(argv: readonly string[], seed: number): PolicyGradientCliOptions {
  const trainingArgs = [
    ...
    '--goal-reward',
    String(numberArg(argv, '--goal-reward', 1)),
    '--win-reward',
    String(numberArg(argv, '--win-reward', 1.4)),
    '--start-state-mode',
...
function appendHistory(path: string, result: PromotionLoopResult): void {
  ...
  appendFileSync(path, `${JSON.stringify({
    ...
    goalReward: result.training.goalReward,
    winReward: result.training.winReward,
...
function trainingFromCandidateMetadata(
  ...
  return {
    ...
    goalReward: finiteMetadataNumber(metadata, 'goalReward', fallback.goalReward),
    winReward: finiteMetadataNumber(metadata, 'winReward', fallback.winReward),
