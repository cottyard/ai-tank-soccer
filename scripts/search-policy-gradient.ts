...
export function parsePolicyGradientSearchArgs(argv: readonly string[]): PolicyGradientSearchOptions {
  ...
  const startStateMode = stringArg(argv, '--start-state-mode') ?? 'mixed';
  const advantageBaseline = stringArg(argv, '--advantage-baseline') ?? 'start-team-time';
  const actionMode = stringArg(argv, '--action-mode') ?? 'runtime';
  const opponentMode = stringArg(argv, '--opponent-mode') ?? 'self';
...
export function runPolicyGradientSearch(
  ...
  const rows = options.variants.map((variant, index) => {
    const variantId = `v${String(index + 1).padStart(2, '0')}-lr${slugNumber(variant.learningRate)}-e${variant.epochs}-clip${slugNumber(variant.ppoClip)}-t${slugNumber(variant.temperature)}`;
    ...
    return {
      ...
      training,
      standard: evaluateGate(currentWeights, candidateWeights, options, evaluate)
    };
...
function evaluateGate(
  ...
  const seeds = options.standardSeeds.map((seed) => {
  ...
  const current = aggregateEvaluations(seeds.map((seed) => seed.current));
  const candidate = aggregateEvaluations(seeds.map((seed) => seed.candidate));
  return {
    current,
    candidate,
    delta: deltaEvaluation(candidate, current),
...
function compareRows(a: PolicyGradientSearchRow, b: PolicyGradientSearchRow): number {
  return b.standard.delta.score - a.standard.delta.score ||
    ...
    for (const epochs of grid.epochs) {
      for (const ppoClip of grid.ppoClips) {
        for (const temperature of grid.temperatures) {
          variants.push({ learningRate, epochs, ppoClip, temperature });
        }
      }
    }
  }
